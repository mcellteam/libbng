...
bool RxnClass::is_absorb_region_border_type_incl_all_molecules() const {
  bool res = (!rxn_rule_ids.empty() && all_rxns.get(rxn_rule_ids[0])->is_absorptive_region_rxn());
  if (res) {
    // this is a special case where we allow only a single rxn in this class
    release_assert(rxn_rule_ids.size() == 1);
  }
  return res;
}
